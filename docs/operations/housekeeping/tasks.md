...
| Task Type | Yes | Type of task — from [HK Task](/configuration/housekeeping-setup/tasks) setup. |
...
Low → Medium → High → Urgent → Critical. Use Urgent or Critical for tasks that must be done immediately (e.g. VIP arrival room).
...
Yes — select multiple rooms in the Floor/Room multi-select when creating the task. All selected rooms are saved on a single task record, so updating, assigning, or completing the task applies to all of its rooms at once.
...
- [Room Status](/operations/housekeeping/room-status) — Update room status after task completion.
- [Configuration → HK Tasks](/configuration/housekeeping-setup/tasks) — Set up task type codes.
- [Configuration → HK Sections](/configuration/housekeeping-setup/sections) — Floor/section setup.
- [Configuration → HK Attendants](/configuration/housekeeping-setup/attendances) — Housekeeping staff setup.
