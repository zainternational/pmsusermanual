...
At the top, five summary cards show: **OOO/OOS** (rooms currently out of order or out of service), **Total Tasks**, **Pending**, **Completed**, and **Urgent/Critical** (records with Urgent or Critical priority). There is no separate "In Progress" state — a task is only ever Pending or Completed.

## Status Filter

A **Status** filter above the table lets you switch between **Pending**, **Complete**, and **All** maintenance records. The tab opens with the filter set to **Pending** by default, so newly created or unresolved maintenance tasks are shown first.
...
| Maintenance Type | No | Type of maintenance work — from [Maintenance Types](/configuration/housekeeping-setup/management-types) setup. |
...
:::note
If no out-of-order rooms appear in the dropdown, either all OOO rooms already have pending maintenance tasks, or no rooms are currently marked as out of order. Go to the [Out of Order](/operations/housekeeping/out-of-order) tab to check.
:::
...
The maintenance record remains in the history. The room's OOO status is cleared separately from the [Out of Order](/operations/housekeeping/out-of-order) tab using the Return to Service action.

## See also

- [Out of Order](/operations/housekeeping/out-of-order) — Mark rooms as OOO before creating maintenance tasks; return rooms to service after maintenance.
- [Room Status](/operations/housekeeping/room-status) — Room status reflects OOO/OOS while maintenance is in progress.
- [Configuration → Maintenance Types](/configuration/housekeeping-setup/management-types) — Set up maintenance type codes.
- [Configuration → Out of Order Reasons](/configuration/housekeeping-setup/out-of-order-reasons) — Set up OOO reason codes used as the maintenance title.
